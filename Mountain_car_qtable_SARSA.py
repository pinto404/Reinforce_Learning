...
def update_q_table(state, action, reward, next_state, next_action):
    if next_action != None:
        loss = (reward + gamma * (q_table[next_state[0]][next_state[1]][next_action]) - q_table[state[0]][state[1]][action])
    else:
        loss = reward + gamma * (0 - q_table[state[0]][state[1]][action])
...
for episode in range(2000):
    ...
    while(True):
        ...
        #next_stateが終端の場合、行動をとれないのでNone
        if done or truncated:
            total_loss += update_q_table(state, action, reward, next_state, None)
            break
        next_action = select_action(next_state, q_table, epsilon)
        total_loss += update_q_table(state, action, reward, next_state, next_action)
        state = next_state
        action = next_action
...
for test_episode in range(5):
    ...
    while(True):
        ...
        if done or truncated:
            break
        next_state = (select_pos_index(next_state[0]), select_velo_index(next_state[1]))
        next_action = select_action(next_state, q_table, epsilon=0)
        state = next_state
        action = next_action
